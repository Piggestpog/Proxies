...
if(requestURL.indexOf(addLink) != -1){  // 添加外部播放器链接
  ...
  if(obj.ExternalUrls){ // 兼容终点站的原生 url schema 连接
    obj.ExternalUrls = obj.ExternalUrls.filter(function(item) {
      return !(item.Name.indexOf("PotPlayer") != -1 ||item.Name.indexOf("Infuse") != -1 || item.Name.indexOf("nPlayer") != -1 || item.Name.indexOf("VLC") != -1 || item.Name.indexOf("IINA") != -1 || item.Name.indexOf("Movist Pro") != -1);
    });
  }

  obj.ExternalUrls = [...obj.ExternalUrls, ...infusePlay, ...nplayerPlay, ...shuDownload];
